...
async function run() {
  ...
  try {
    ...
    const { body, headers } = buildMultipartPayload({}, {
      foto: { filename: 'test_kades.jpg', contentType: 'image/jpeg', content: validJpegContent }
    });
    ...
    assert.strictEqual(uploadRes.status, 200, 'Upload image must return HTTP 200');
    const uploadData = await uploadRes.json();
    assert.strictEqual(uploadData.success, true, 'Upload response success must be true');
    assert.ok(uploadData.url.startsWith('/uploads/'), 'Returned URL must be relative /uploads/ path');
    assert.ok(uploadData.url.endsWith('.jpg') || uploadData.url.endsWith('.jpeg'), 'Returned URL must preserve image extension');
    ...
    assert.strictEqual(fs.existsSync(savedFilePath), true, 'Uploaded file must physically exist in uploads directory');
    ...
    const fakePayload = buildMultipartPayload({}, {
      foto: { filename: 'malicious.php', contentType: 'application/x-php', content: fakePhpContent }
    });
    ...
    assert.ok(rejectRes.status === 400 || rejectRes.status === 500, 'Non-image file must be rejected');
    ...
    assert.strictEqual(emptyRes.status, 400, 'Uploading with no file must return HTTP 400');

    // Cleanup file test jika ada
    try { if (fs.existsSync(savedFilePath)) fs.unlinkSync(savedFilePath); } catch (e) {}
  } finally {
...
if (require.main === module) {
  run().then(() => console.log('File Upload Feature Test: PASS')).catch(e => {
