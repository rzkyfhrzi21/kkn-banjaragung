...
app.use(cors({
  origin: (origin, cb) => {
    ...
    const safe = /^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?$|^https:\/\/banjaragung-tanggamus\.web\.id$/.test(origin);
    return cb(null, safe);
  }
}));
...
app.get('/sitemap.xml', (req, res) => {
  ...
  const urls = [];
  try {
    const files = fs.readdirSync(PUBLIC_DIR).filter(f => f.endsWith('.html') && f !== 'admin.html').sort();
    for (const f of files) {
      const page = f === 'index.html' ? '' : f;
      const priority = f === 'index.html' ? '1.0' : (['profil.html', 'pemerintahan.html', 'layanan-publik.html', 'kontak.html'].includes(f) ? '0.8' : '0.6');
